<template scoped>
	<div style="position:relative;">
		...
		<div class="d-flex justify-content-between" style="height: 82.4074074vh; width: 94.354166vw; margin-left: 4.177083vw;">
			...
			<div class="leftside align-self-center bg-white d-flex flex-column">
				...
				<div class="pieChartDiv align-self-center mt-3 d-flex">
					...
					<div class="d-flex flex-column justify-content-center" style="width:30%; height:100%;">
						...
						<blueBtn class="align-self-center mt-5" text="輸入費率" :func="ToggleCostRateModal"></blueBtn>
					</div>
				</div>
				<!-- table -->
			...
			<div class="rightside align-self-center d-flex flex-column justify-content-between">
				<div class="upper-right bg-white">
					...
					<div class="barChartDiv d-flex justify-content-between">
						...
						<div class="chartLegend align-self-center d-flex flex-column justify-content-around" style="width:20%; height:95%;">
							<blueBtn class="align-self-center mt-3" text="查詢區間用電" :func="ToggleIntervalPCModal"></blueBtn>
				...
				<div class="lower-right bg-white">
					<subtitle :subtitleInfo="subtitle.powerFactor" :func="ToggleRunningStateModal"></subtitle>
		...
		<modalCostRate v-if="costRateModal" :closeBtn="this.ToggleCostRateModal"></modalCostRate>
		<modalSearch v-if="searchIntervalPCModal" :chartLegend="modalLegend.intervalPC" headerText="查詢區間用電" title="用電需量" chartType="line" :closeBtn="this.ToggleIntervalPCModal"></modalSearch>
		<modalSearch v-if="searchRunningStateModal" :chartLegend="modalLegend.runningState" headerText="查詢運行狀態" title="運行狀態" chartType="grant" :closeBtn="this.ToggleRunningStateModal"></modalSearch>
	</div>
</template>
...
import modalCostRate from './child/modalCostRate.vue';
import modalSearch from './child/modalSearch.vue';

export default {
	name: "PowerAnalysis",
	components: {asideNav,subtitle, settingBoard, pieChart, barChart, lineChart, tableAnalyze, capsule, blueBtn, modalCostRate, modalSearch},
	data(){
		return{
			costRateModal: false,
			searchIntervalPCModal: false,
			searchRunningStateModal: false,
			subtitle:{
				costimate:{
	...
	methods: {
		ToggleCostRateModal: function(){ this.costRateModal = !this.costRateModal;},
		ToggleIntervalPCModal: function(){ this.searchIntervalPCModal = !this.searchIntervalPCModal;},
		ToggleRunningStateModal: function(){ this.searchRunningStateModal = !this.searchRunningStateModal;}
	}
}
</script>
